...
if [ "$getDrives" == "1" ]; then
	...
	slat_mean=$(grep -A 3 "slat_ns"  fio_test_10.json | grep "mean" | grep -v "0.000" | awk '{print $3}' | sed 's/,//g') 
	clat_mean=$(grep -A 3 "clat_ns"  fio_test_10.json | grep "mean" | grep -v "0.000" | awk '{print $3}' | sed 's/,//g')
