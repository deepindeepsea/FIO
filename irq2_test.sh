...
cat /proc/interrupts | grep "CPU0\|nvme0q\|nvme1q"  > RR_Gen5_1C1T_${nvme}.begin

grep "nvme0q\|nvme1q" RR_Gen5_1C1T_${nvme}.begin | awk '{for(i=2;i<=64;i++) {if ($i > 10000) {print "#Interrupts "$i " : Core#" i-2 "  NVMEq: "$68  }}}'
...
./fio_nvme_scaled_auto_thread.sh ${nvme} randread $numjob $core
...
cat /proc/interrupts | grep "CPU0\|nvme0q\|nvme1q" > RR_Gen5_1C1T_${nvme}.end
#grep "${nvme}q" RR_Gen5_1C1T.end | awk '{for(i=2;i<=256;i++) {if ($i > 10000) {print "NVMEq: " $260 "  Core#" i-2 " #Interrupts "$i }}}'
grep "nvme0q\|nvme1q" RR_Gen5_1C1T_${nvme}.end | awk '{for(i=2;i<=64;i++) {if ($i > 10000) {print "#Interrupts "($i+3) " : Core#" i-2"  NVME: " $68 }}}'
